...
async function getTitle(link, page, key) {
  ...
  const dataPage = await page.evaluate(() => {
    ...
    // @ts-ignore
    const imageUrlSelector = document.querySelector('#contents > div > div.columnSection.clearfix > div > div.imgSection > div > div.thumb.js_thumb > div.inner > div > ul:nth-child(1) > li:nth-child(1) > img');
    const imageUrl = (imageUrlSelector) ? imageUrlSelector.getAttribute('data-src') : 'undefined';
  ...
  if (dataPage.imageUrl) await newSaveImage(dataPage.imageUrl);
  if (dataPage.iconUrl) await newSaveImage(`https://www.cbre-propertysearch.jp/${dataPage.iconUrl}`);
...
async function newSaveImage(link) {
  const fileNameAr = link.split('/');
  const fileName = fileNameAr[fileNameAr.length - 1];
  const folderImages = 'logs/images';
